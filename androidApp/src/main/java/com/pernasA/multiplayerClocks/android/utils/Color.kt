package com.pernasA.multiplayerClocks.android.utils

import androidx.compose.ui.graphics.Color

val ButtonPrimary = Color(0xFF0055D4) // Azul fuerte (Botón principal)
val ButtonSecondary = Color(0xFF007AFF) // Azul brillante (Botón resaltado)
val ButtonTertiary = Color(0xFF66A3FF) // Azul suave (Botón más claro)

val ButtonPrimaryContainer = Color(0xFF003C9D) // Azul oscuro vibrante
val ButtonSecondaryContainer = Color(0xFF004FBB) // Azul intermedio brillante

val BluePrimary = Color(0xFF0000FF) // Azul brillante
